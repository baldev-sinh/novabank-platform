 ...
 *
 * <p>The {@code User} aggregate is responsible for protecting the consistency of user identity and
 * lifecycle. It owns business rules related to:
 *
 * <ul>
 *   <li>User registration
 *   <li>Lifecycle transitions
 ...
 *
 * <ul>
 *   <li>{@link #register(EmailAddress, PasswordHash)} for new users.
 *   <li>{@link #restore(UserId, EmailAddress, PasswordHash, UserStatus, Set, Instant, Instant)} for
 *       reconstructing persisted users.
 * </ul>
 *
 * <p>The constructor remains private to ensure that all instances are created through controlled
 * factory methods.
 */
public final class User {
   ...
   *
   * <p>This factory creates a brand-new {@code User} aggregate by applying the business rules for
   * registration.
   *
   * <p>The newly registered user:
   *
   * <ul>
   *   <li>Receives a new {@link UserId}.
   *   <li>Starts in the {@link UserStatus#PENDING_VERIFICATION} state.
   *   <li>Is assigned the {@link RoleName#CUSTOMER} role.
   *   <li>Initializes creation and update timestamps.
   * </ul>
   *
   ...
   *
   * <p>This factory is intended for use by the persistence layer when restoring an aggregate from
   * storage. Unlike {@link #register(EmailAddress, PasswordHash)}, This factory reconstructs an
   * existing aggregate from persisted state without reapplying business rules.
   *
  ...
  public static User restore(
      ...
      Instant createdAt,
      Instant updatedAt) {
    return new User(id, email, passwordHash, status, roles, createdAt, updatedAt);
  }

  /**
   * Activates a user after successful verification.
   *
   * <p>Only users in the {@link UserStatus#PENDING_VERIFICATION} state may be activated.
   *
   * @throws DomainException if the current state does not allow activation
   */
  public void activate() {
    if (status != UserStatus.PENDING_VERIFICATION) {
      throw new DomainException("Only users pending verification can be activated.");
    }
  ...
  public void removeRole(RoleName roleName) {
    ...
    if (this.roles.size() == 1) {
      throw new DomainException("User must have at least one assigned role.");
    }
